...
class Mask2FormerRelation(Mask2Former):
    ...
    def forward_train(self,
        ...
        rel_input_list = torch.cat(
            rel_input_list, dim=0)
        rel_target_list = torch.cat(rel_target_list, dim=0)
        rel_output = self.relation_head(
            rel_input_list, mask_attention)
        loss_relation = self.relation_head.loss(
            rel_output, rel_target_list, mask_attention)
